...
class EventLoopKernel:
    ...
    def run(self, ctx: RuntimeContext) -> Dict[str, Any]:
        ...
        if prev_data is None:
            ctx.obs.on_run_end(final_time=0, final_oms_view=ctx.oms.view(), error="No data")
        ...
        ctx.obs.on_run_end(final_time=self._t_cur, final_oms_view=ctx.oms.view(), error=None)
        return ctx.obs.get_run_result()
