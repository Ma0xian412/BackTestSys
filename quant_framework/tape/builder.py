...
class UnifiedTapeBuilder(ITapeBuilder):
    ...
    def __init__(
        self,
        config: TapeConfig = None,
        tick_size: float = 1.0,
        queue_resolver: QueueConstraintResolver = None,
    ):
        ...
        self.queue_resolver = queue_resolver or QueueConstraintResolver(epsilon=EPSILON)
    ...
    def build(self, prev: NormalizedSnapshot, curr: NormalizedSnapshot) -> List[TapeSegment]:
        ...
        segments = self.queue_resolver.derive_cancellations_and_net_flow(segments, prev, curr)
